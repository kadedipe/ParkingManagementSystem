# ============================================================================
# Main Application - Parking Service Entry Point (Simplified)
# ============================================================================
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    try:
        if settings.ENVIRONMENT == "test":
            logger.info("Test environment: database initialization handled by test fixtures.")
        elif settings.AUTO_CREATE_TABLES:
            logger.info("AUTO_CREATE_TABLES=true: initializing database schema")
            await init_db()
        else:
...
async def readiness_check():
    try:
        ...
        return JSONResponse(status_code=503, content={"status": "not_ready", "service": settings.SERVICE_NAME})

@app.get("/", tags=["root"])
async def root():
    return {
        "message": "Parking Service API",
        "version": settings.VERSION,
        "environment": settings.ENVIRONMENT
    }
...
# Import and include the v1 router (which includes auth, parking_lots, etc.)
try:
    from src.api.v1.routes import router as v1_router
    # IMPORTANT: No prefix here - the v1 router already has /v1 prefix
    app.include_router(v1_router)
    logger.info("V1 router loaded successfully")
except ImportError as e:
    logger.warning(f"V1 router not available: {e}")

# Import and include the v2 router for API versioning
try:
    from src.api.v2.routes import router as v2_router
    app.include_router(v2_router)
    logger.info("V2 router loaded successfully")
except ImportError as e:
    logger.warning(f"V2 router not available: {e}")
